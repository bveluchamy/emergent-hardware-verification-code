// cdc_handshake_rx_mut.sv -- MUTATION of the Chapter 2 design (one realistic bug);
// proven against the unchanged checker by the Chapter 3 engines -> the bug is caught.
// THE BUG: sync_ack_b is stuck at 0 (the ack signal is never driven), so a request is
// never acknowledged and the RX 'answers req with ack' guarantee is starved.
module cdc_handshake_rx (
  ...
  always_ff @(posedge clk_b or negedge rst_b_n) begin
    if (!rst_b_n) begin
      ...
      state <= next_state;
      sync_ack_b <= 1'b0;  // BUG: ack never driven
