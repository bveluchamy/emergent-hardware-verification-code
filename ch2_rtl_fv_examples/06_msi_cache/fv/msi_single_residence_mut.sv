// msi_single_residence_mut.sv -- MUTATION of the book MSI cache: cpu_hit is broken to
// constant 0, so the controller never detects an existing copy and a repeated (tag,set)
// is filled into two different victim ways -- a single-residence violation the bounded
// BMC check catches.
...
module msi_cache_node #(
  ...
  always_comb begin
    ...
    cpu_hit   = 1'b0;  // BUG: hit detection disabled
