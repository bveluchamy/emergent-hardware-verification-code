// sync_fifo_word_mut.sv -- MUTATION of the book FIFO (the memory write drops its
// data: mem[wr] <= '0 instead of req.wdata). The word-level data-integrity checker
// (read-over-write, symbolic address) refutes it.
...
module sync_fifo #(
  ...
  always_ff @(posedge clk) begin
    if (req.push && !rsp.full)
      mem[wr_ptr[AW-1:0]] <= '0;  // BUG: data dropped
  end
