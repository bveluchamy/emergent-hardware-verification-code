#!/usr/bin/env python3
"""04_sat_engine reference: legal set WITH the nonlinear product constraint.
...
# how many the product constraint removed vs the base 720 (no product)
base = sum(1 for p in permutations(range(1,10),5) if sum(p)==25 and len(set(p))==5 and p[0]<p[1])
print(f"  (base without product: {base}; product removed {base-len(sols)})")
